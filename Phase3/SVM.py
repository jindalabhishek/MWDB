...
def compute_alpha_j(alpha, Y_j, E_i, E_j, eta, L, H):
    ...
    if(new_alpha > H): return H
    elif(new_alpha < L): return L
    else: return new_alpha
...
def compute_threshold(b, C, \
    ...
    b_1 = b - E_i - (Y_i)*(alpha_i - old_alpha_i)*np.inner(X_i, X_i) - (Y_j * (alpha_j - old_alpha_j) * np.inner(X_i, X_j))
    b_2 = b - E_j - (Y_i)*(alpha_i - old_alpha_i)*np.inner(X_i, X_j) - (Y_j * (alpha_j - old_alpha_j) * np.inner(X_j, X_j))

    if(alpha_i > 0 and alpha_i < C): return b_1
    elif(alpha_j > 0 and alpha_j < C): return b_2
    else: return (b_1 + b_2)/2.
    ...
    while(iteration < max_iteration):
      ...
      for i in range(0,n_samples):
        ...
        if((Y[i]*E_i < -eps and alpha[i] < C) or (Y[i]*E_i > eps and alpha[i] > 0)):
          ...
          threshold = compute_threshold(threshold, C, X[i], X[j], Y[i], Y[j], E_i, E_j, alpha[i], alpha[j], old_alpha[i], old_alpha[j])
          n_changed_alpha += 1
      if(n_changed_alpha == 0): iteration += 1
      else: iteration = 0
...
def multiclass_classifier(X, train_set, tie_mode = 0):
    ...
    for i in range(0, n_samples):
      ...
      for class_i in range(0,n_classes):
        votes[class_i] = binary_classifier(classifiers[class_i]['alpha'], classifiers[class_i]['threshold'], classifiers[class_i]['W'],X[i], X, classifiers[class_i]['Y_binary'])
      ...
      if(len(classified) > 1): 
        ...
        if(ret[closest] != 'NA'): # Already classified
         if(tie_mode == 0): classified[0] = np.where(classes == ret[closest])[0][0]
         elif(tie_mode == 1):
          for j in classified:
